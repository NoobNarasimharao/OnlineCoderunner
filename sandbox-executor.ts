...
(global as any).eval = () => { throw new Error('eval() is not allowed'); };
(global as any).Function = () => { throw new Error('Function constructor is not allowed'); };
    ...
    try {
      ...
      req.on('end', async () => {
        try {
          ...
          const child = spawn('ts-node', [filepath], {
            cwd: tempDir,
...
server.listen(PORT, '0.0.0.0', () => {
  console.log(`TypeScript sandbox executor running on port ${PORT}`);
});
